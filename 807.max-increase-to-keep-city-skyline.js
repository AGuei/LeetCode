...
var maxIncreaseKeepingSkyline = function (grid) {
  ...
  for (row = 0; row < len; ++row) {
    rowMax[row] = grid[row][0];
    colMax[row] = grid[0][row];
    for (col = 1; col < len; ++col) {
      rowMax[row] = Math.max(grid[row][col], rowMax[row]);
      colMax[row] = Math.max(grid[col][row], colMax[row]);
    }
  }
  for (row = 0; row < len; ++row) {
    for (col = 0; col < len; ++col) {
      sumHeight += Math.min(rowMax[row], colMax[col]) - grid[row][col];
    }
  }
  return sumHeight;
};
